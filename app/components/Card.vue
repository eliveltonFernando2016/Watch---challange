<template>
  ...
  >
    <span v-if="props.cardType === 'artist'" class="font-roboto font-bold text-lg leading-5.5 text-white absolute bottom-13.5 text-center w-52">{{ props.name }}</span>
    <template v-else-if="props.cardType === 'musicalStyle'">
      ...
      <div class="absolute px-4 bottom-8">
        <p class="font-roboto font-bold text-[32px] text-white">{{ props.name }}</p>
    ...
    <template v-else-if="props.cardType === 'live'">
      ...
      <div class="px-4 py-3 flex items-center justify-between">
        <span class="font-roboto text-sm text-primary">{{ props.name }}</span>
        <div class="font-roboto text-sm text-[#FF2828] leading-4 flex items-center justify-between gap-2">
          <span class="size-2 rounded-full bg-[#FF2828] float-left animate-pulse" />
          Live
        </div>
      </div>
      <div class="px-4 flex items-center justify-between">
        <span class="font-roboto text-sm text-secondary leading-4">{{ props.stage }}</span>
        <span class="font-roboto text-sm text-secondary leading-4">{{ DateFormat(props.duration as Date) }}</span>
      </div>
    </template>
